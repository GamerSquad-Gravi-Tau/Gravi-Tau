...
public class FinalBossShootGun : MonoBehaviour
{
    GameObject Player;
    ...
    void Update()
    {
        if (Dead)
        {
            if (Right)
            {
                this.GetComponentInParent<FinallBossBehavior>().RightShootGunAlive = false;
            }
            else
            {
                this.GetComponentInParent<FinallBossBehavior>().LeftShootGunAlive = false;
            }
